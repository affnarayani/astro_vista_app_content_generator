...
def get_monthly_horoscope(zodiac_sign):
    """
    Fetch monthly horoscope for the given zodiac sign from ProKerala website.
    ...
    # Construct the URL
    url = f"https://www.prokerala.com/astrology/monthly-horoscope/{zodiac_sign}.html"
    ...
    if h1_tag:
        ...
        # Extract only the first part of the title before the "•" symbol
        if "•" in full_title:
            result['title'] = full_title.split("•")[0].strip()
        else:
...
def main():
    ...
    for i, sign in enumerate(zodiac_signs):
        ...
        horoscope_data, _ = get_monthly_horoscope(sign)
    ...
    # Save to JSON file
    with open('monthly_horoscope.json', 'w', encoding='utf-8') as f:
