...
def main():
    ...
    with open('weekly_horoscope.json', 'r') as file:
    ...
    for sign_data in horoscope_data['horoscopes']:
        ...
        for i, paragraph in enumerate(sign_data['content']):
            ...
            section_name = f"weekly horoscope paragraph {i+1}"
    ...
    with open('modified_weekly_horoscope.json', 'w') as file:
        json.dump(modified_horoscope_data, file, indent=4)
    
    print("\nAll weekly horoscopes successfully modified and saved to 'modified_weekly_horoscope.json'")
